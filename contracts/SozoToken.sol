...
import './huzzle-framework/Token.sol';


/// @title Token contract - Implements Standard ERC20 Token for Sozo project.
contract SozoToken is Token {

	/// TOKEN META DATA
	string constant public name = 'SOZO Global';
	string constant public symbol = 'TUSI';
	...
	/*** Tokens reserved for the SOZO team ***/
	...
	{   
		...
		if (_owner == teamAllocation) {
			...
			spentTokens = balanceOf(teamAllocation) < teamTotal ? teamTotal - balanceOf(teamAllocation) : 0;
			...
			spentTokens = balanceOf(advisorsAllocation) < advisorsTotal ? advisorsTotal - balanceOf(advisorsAllocation) : 0;
		} else {
			return allowed[_owner][_spender];
		}
		...
		returns (uint256) 
	{
		...
		return add(_unlockedAfterCliff, mul(periods, _periodAmount));
	}
}
